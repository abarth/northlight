...
const rows = await page.evaluate(async (GROUP) => {
  ...
  for (const preset of group.presets) {
    ...
    const s = new NL.StrokeSession(eng, preset.settings,
      { fg: { h: 0, s: 0, v: 0 }, bg: { h: 0, s: 0, v: 1 }, rng: mk(4242) });
    s.down(pts[0]);
    s.move(pts.slice(1));
    s.up();
    eng.endStroke('bg');
    ...
    if (total === 0) { out.push({ name: preset.name, coverage: 0, density: 0, ink: 0, band: 0 }); continue; }
...
console.log(`\n${'brush'.padEnd(22)} coverage  density    ink   band`);
console.log('-'.repeat(58));
for (const r of rows) {
  const bar = '#'.repeat(Math.round(r.coverage * 24));
  console.log(`${r.name.padEnd(22)}   ${(r.coverage * 100).toFixed(0).padStart(3)}%    ` +
              `${r.density.toFixed(2)}   ${r.ink.toFixed(2)}  ${String(r.band).padStart(4)}px  ${bar}`);
}
await browser.close();
server.kill();
process.exit(0);
